...
//still old, min crossing angle not added
...
public class GraphTestFixedRandom {
		//fixed means that the repeated times are pre decided
		//means that algorithm stops when the repeated times are reached.

		//in this program, the random graphs are generated after the program is running,
		//therfore this is a need to save the initial layouts: goStr
	...
	public static void main(String[] args)
	{
		...
		for (int m=1; m<2; m++)
		{
			...
			String strResults = "size density cross angleM angleD edgeM edgeD minVertx finVertx devVertx ";

			strResults += "cross1 cross2 angleM1 angleM2 angleD1 angleD2 ";
			...
			String g1Str="===Resulting graphs based on new method, with acute force; " + strModel +" model \n";
			...
			for (int i = 0; i <graphNumber; i++)
			{
				...
				//new method with acute force
				g1.COSINE_STRENGTH = 0.0;
				g1.ROTATE_STRENGTH = 1.0;
				...
				else
				{
					...
					strResults +=  g0.getNumOfCrosses() + ","+ g0.getDegreeMean() +","+ g0.getDegreeDeviation() +","+g0.getEdgeMean()+","+g0.getEdgeDeviation()+",";
			...
			strMedian +=  median(edgeM1)+","+median(edgeM2)+","+median(edgeD1)+","+median(edgeD2)+","+median(minVert1)+","+median(minVert2)+",";
